...
- **Manual-page opt-out of legacy body texture:** `app/globals.css` has unconditional `body::before` (fixed, z-100, grain, multiply blend) and `body::after` (z-85, warm vignette) plus `body { background: var(--ink) }`. Converted pages mark themselves with a root `<div className="manual-root">`; add CSS: `html:has(.manual-root), body:has(.manual-root) { background: var(--ground); } body:has(.manual-root)::before, body:has(.manual-root)::after { display: none; }` (html too — `globals.css:48` sets `html { background-color: var(--ink) }` and the overscroll gutter would stay legacy otherwise). Every converted page in Tasks 7/9/12/15 MUST wrap in `.manual-root` or it renders under the legacy grain/vignette and fails its fidelity gate.
...
- [ ] **Step 1:** Compose a sample chapter-shaped page using every Task 6 primitive, **wrapped in `.manual-root`** (Task 3's body-texture opt-out — without it the page renders under legacy grain/vignette and this gate fails), spine-fragment placeholder copy (public-safe), one `ExplodedStack` demo figure with real ground truth (the portfolio site's own stack: Next/React/Tailwind layers — safe, true, and self-referential).
...
- [ ] **Step 1:** Convert `app/case-studies/[slug]/page.tsx` to ChapterLayout (`.manual-root`) with prev/next from the case-study order in `content/case-studies.ts`; words from `lib/word-counts.ts`; add `<ColophonFooter />` (this route has no footer today). **Mechanical token sweep is part of this step:** `grep -c "text-paper\|bg-ink\|border-rule\|text-accent\|dither-frame\|frame-well" app/case-studies/[slug]/page.tsx` must be 0 when done.
...
- [ ] **Step 0:** **Font prep — Satori cannot read woff2, and `node_modules/geist/dist/fonts/geist-pixel/` ships woff2 ONLY.** Convert `GeistPixel-Square.woff2` → ttf into `public/fonts/geist-pixel/` via `uv run --with fonttools --with brotli python -c "from fontTools.ttLib.woff2 import decompress; decompress('node_modules/geist/dist/fonts/geist-pixel/GeistPixel-Square.woff2','public/fonts/geist-pixel/GeistPixel-Square.ttf')"` (brotli is required — verified working with it, ImportError without). GT Sectra: use the source TTFs already on disk (see `scripts/subset-fonts.py` SRC dir). Fallback if conversion fails: draw the wordmark as SVG rects (pixel-glyph boxes) in the OG JSX; log it.
...
- [ ] **Step 1:** Implement tokens; compute contrast for `--blueprint-bright` on `--ground` dark (document the ratio in a CSS comment). This is also where the structural inversion promised in Task 3 lands: move remaining-relevant light values onto `:root`, put cyanotype under `html.dark`, and convert/remove the now-redundant `html.light` blocks (Task 19 deletes whatever legacy remains).
